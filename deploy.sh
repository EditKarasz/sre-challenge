...
### This is a constans: the 2 applications which are the parts of the service
...
replace_value .metadata.name "${namespace}" temp/namespace.yaml
...
### Apply LimitRange
kubectl apply -f limitrange.yaml --namespace="${namespace}" || error 'Failed to apply limitrange config'

## Check if exists
kubectl describe limits small --namespace="${namespace}" || error 'Limitrage does not exit'
...
do
  ...
  kubectl apply -f temp/deployment.yaml --namespace="${namespace}" || error "Failed to apply ${application} config"

  ### Check if exists
  #### kubectl exits with error if it does not exist
  kubectl get deployment --namespace="${namespace}" --selector app="${application}"
  ...
  ### Deploy the service
  kubectl apply -f temp/service.yaml --namespace="${namespace}" || error "Failed to apply ${application} config"

  ### Check if exists
  #### kubectl exits with error if it does not exist
  kubectl get services --namespace="${namespace}" --selector app="${application}"
  ...
  then
    ...
    sleep 60
    ## Collect EXTERNAL-IP:
    external_ip=$(kubectl get services --namespace="${namespace}" --selector app="${application}" -o yaml | yq  .items[0].status.loadBalancer.ingress.[0].ip)
